...
#endif
{
    ...
    public interface IDbContextValidator
    {
        ...
        /// <returns>A collection of <see cref="InvalidMapping"/>s, i.e. when an entity defined in the model does not have a matching table and column names in the database. If the context model exactly matches the actual database then an empty collection is returned.</returns>
        Task<IReadOnlyCollection<InvalidMapping>> ValidateContextAsync(DbContext context, IProgress<float> progress = null, CancellationToken cancellationToken = default);
    }
}